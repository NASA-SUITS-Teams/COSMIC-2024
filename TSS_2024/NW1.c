...
void get_ip_address(char* hostname_out) {
    ...
    // Resolve the local host name to an IP address
    ...
    // Loop through the addresses and find a non-local host IPv4 address
    for (ptr = result; ptr != NULL; ptr = ptr->ai_next) {
        struct sockaddr_in* sockaddr_ipv4 = (struct sockaddr_in*)ptr->ai_addr;
        const char* ip_address = inet_ntoa(sockaddr_ipv4->sin_addr);

        // Check if it's not localhost
        if (strcmp(ip_address, "127.0.0.1") != 0) {
            strcpy(hostname_out, ip_address);
            break;
        }
    }
